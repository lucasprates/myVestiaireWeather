...
class OpenWeatherAPI: NSObject, WebServiceResponseDelegate {
    ...
    private let openWeatherPath = "api.openweathermap.org/data/2.5/forecast/daily?q=Paris&mode=json&units=metric&cnt=16&APPID=648a3aac37935e5b45e09727df728ac2"
    ...
    func didReceiveResponse(response: WebServiceResponse, fromRequest request: WebServiceRequest) {
        ...
        if response.status == .ok {
            ...
            if let jsonDictionaryData: [String : Any] = (response.body as? [String : Any]){
                ...
                //also, tell the upper class that new information was received from Open Weather API Servers
                self.newDataArrivedReturnBlock?(newForecasts)
            }
        } else {
            //also, tell the upper class that no information was received from Open Weather API
            self.newDataArrivedReturnBlock?(nil)
        }
    }
}
